...
class Organ:
    ...
    def _draw_convex_hull(self, im, points, color):
        ...
        # interpolation
        points = utils.mask_interpolation(points.squeeze())[:, None, :]
        cv2.fillConvexPoly(im, points.astype(np.int32), color=color)
    ...
    def whitening(self, rate=0.03, confirm=True):
        '''
        提亮美白
        arguments:
    ...
    def brightening(self, rate=0.1, confirm=True):
        '''
        提升鲜艳度
        arguments:
    ...
    def smooth(self, rate=0.3, ksize=None, confirm=True):
        '''
        磨皮
        arguments:
        ...
        if confirm:
            self.confirm()
            patch = self.patch_bgr.copy().astype(np.float32)
            temp1 = cv2.bilateralFilter(patch, dx, fc, fc)
            temp2 = temp1 - self.patch_bgr + 128
            temp3 = cv2.GaussianBlur(temp2, (2 * value2 - 1, 2 * value2 - 1), 0, 0)

            patch_new = self.patch_bgr + 2 * temp3 - 255
    ...
    def sharpen(self, rate=.5, confirm=True):
        ...
        if confirm:
            ...
            src = self.patch_bgr.copy().astype(np.float32)
            # get high contrast image

            # highcontrast = utils.adjustContrast(src, contrast)
            # get blur image
            blurimage = cv2.GaussianBlur(src, (0, 0), sigma)
            # sub
            diffimage = np.abs(src - blurimage)

            # sharpen
            mask = diffimage < threshold
            # cv2.imwrite('temp.jpg', mask.astype(np.uint8) * 255)
            sharp = src.copy()
            sharp[mask] = src[mask] * (1 + amount) - blurimage[mask] * amount
...
class Makeup():
    ...
    def get_faces(self, im_bgr, im_hsv, temp_bgr, temp_hsv, name, n=1):
        ...
        for i, rect in enumerate(rects):
            landmark = np.array([[p.x, p.y] for p in self.predictor(im_bgr, rect).parts()])
            faces.append(Face(im_bgr, im_hsv, temp_bgr, temp_hsv, landmark, i))
...
if __name__ == '__main__':
    ...
    for face in faces[path]:
        ...
        face.organs['forehead'].smooth(.35)
        face.organs['mouth'].brightening()
        # face.organs['mouth'].smooth(smooth_rate)
        ...
        face.organs['nose'].smooth(smooth_rate)
        face.organs['nose'].sharpen()
        # face.sharpen()
        face.sharpen()
    cv2.imwrite('./processed/orig.jpg', imc)
    cv2.imwrite('./processed/proc.jpg', im)

    # transform
    shape = landmarks[0].astype(np.float32)
    print(shape.shape)
    image_transformed, transform_matrix = preprocess.image_transform(im, shape)
    shape = cv2.perspectiveTransform(shape[None, ...], transform_matrix).squeeze()
    # segmentation background
    im2, mask = preprocess.segm_bg(image_transformed)
    cv2.imwrite('./processed/proc_cut.jpg', im2)
    print('Time: %.4fs' % (time() - t))
    print('Quiting')
